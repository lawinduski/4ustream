...
type AppContextType = { user: User|null; profile: UserProfile|null; loading: boolean; lang: Lang; setLang:(l:Lang)=>void; theme:Theme; setTheme:(t:Theme)=>void; t:(key:keyof typeof translations.en)=>string; install:()=>Promise<void>; canWatch:boolean; };
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const value=useMemo(()=>({user,profile,loading,lang,setLang,theme,setTheme,t,install,canWatch:!!user&&profile?.status==='active'}),[user,profile,loading,lang,theme]);
