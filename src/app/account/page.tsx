...
export default function Account(){
 ...
 return <PageShell><div className="max-w-4xl mx-auto space-y-6">
   <div className="glass rounded-3xl p-7 sm:p-10"><div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-violet-500 to-cyan-400 grid place-items-center"><UserRound/></div><h1 className="text-3xl font-black mt-5">{profile?.name||user.displayName||'User'}</h1><p className="text-slate-400 mt-1">{user.email}</p><div className="mt-7 grid sm:grid-cols-2 gap-3"><div className="p-4 rounded-2xl bg-white/5 border border-white/10"><div className="text-xs text-slate-500">پلانی من</div><div className="font-bold mt-1">{profile?.plan==='vip'?'VIP':'FREE'}</div></div><div className="p-4 rounded-2xl bg-white/5 border border-white/10"><div className="text-xs text-slate-500">VIP</div><div className="font-bold mt-1 text-violet-300">{profile?.plan==='vip'?'چالاک':'ناچالاک'}</div></div></div><div className="flex flex-wrap gap-3 mt-7"><button onClick={install} className="inline-flex items-center gap-2 px-4 py-3 rounded-xl bg-white/5 border border-white/10"><Download size={17}/>{t('install')}</button><button onClick={async()=>{await signOut(auth);router.push('/');}} className="inline-flex items-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 border border-red-400/20 text-red-200"><LogOut size={17}/>{t('logout')}</button></div></div>
